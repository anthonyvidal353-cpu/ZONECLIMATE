      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-6 border-b border-border/50">
        ...
        <div className="flex items-center gap-3">
          <Select value={zoneId} onValueChange={setZoneId}>
            <SelectTrigger data-testid="schedule-zone-select" className="w-[200px] bg-black/40 border-border/70 rounded-full">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="bg-[#121212] border-border/70">
              ...
              <div className="grid grid-cols-2 gap-4 py-2">
                <div>
                  <Label className="text-xs text-zinc-400">Début</Label>
                  <Input
                    data-testid="slot-start-input"
                    type="time"
                    value={form.start}
                    onChange={(e) => setForm({ ...form, start: e.target.value })}
                    className="mt-1 bg-black/40 border-border/70"
                  />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  <Label className="text-xs text-zinc-400">Consigne (°C)</Label>
                  <Input
                    ...
                    className="mt-1 bg-black/40 border-border/70 font-mono-num"
                  />
                </div>
              </div>
              <DialogFooter>
      ...
      <div className="flex gap-1 p-4 overflow-x-auto border-b border-border/40">
        {DAYS.map((d, i) => (
          <button
            ...
            style={{
              background: day === i ? "#FAFAFA" : "transparent",
              color: day === i ? "#0A0A0A" : "#A1A1AA",
            }}
          >
            {d}
          </button>
        ...
        <AnimatePresence mode="popLayout">
          ...
          {daySlots.map((s) => (
            <motion.div
              ...
              className="flex items-center justify-between rounded-md border border-border/50 bg-black/30 px-4 py-3"
            >
              <div className="flex items-center gap-4">
                <Clock weight="duotone" size={20} className="text-zinc-400" />
