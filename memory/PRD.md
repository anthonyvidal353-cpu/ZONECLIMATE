...
- 2026-07-10 : Vue Plénum + Algorithme de régulation. Nouvel onglet « Plénum » (PlenumView) : schéma du plénum de soufflage, état/position des registres motorisés (ouvert/fermé), flux d'air animé, températures/consignes par zone, statut du gainable (production/veille/purge). Algorithme de régulation dans /simulate/tick : demande par zone + hystérésis registres (deadband 0,5°C), consigne modulée envoyée au gainable proportionnellement à la demande max, ventilation adaptée (faible/moyenne/forte), et PURGE ventilation 30 s registres ouverts avant arrêt quand toutes les zones sont satisfaites. tick renvoie désormais {zones, system}. Validé via curl (modulation + purge) + capture Plénum.
- 2026-07-11 : Refonte visuelle du Plénum (schéma conforme au croquis client : plénum vertical à gauche, vannes motorisées + barres dégradées par zone OUVERT/FERMÉ/OFF, nombre de vannes adaptatif).
- 2026-07-11 : ÉTAPE A — Pilotage LOCAL (isolation cloud / Raspberry Pi). Nouveau `backend/tuya_local.py` (tinytuya) : récupération des local_key via le cloud (1x), scan LAN (deviceScan), lecture status + envoi DP en direct sur le LAN. Endpoints super_admin : POST /admin/tuya/local/sync-keys, GET /admin/tuya/local/devices, POST /admin/tuya/local/scan, POST /admin/tuya/local/test. local_key CHIFFRÉE (Fernet, jamais exposée). Collection `local_devices` ajoutée à la sauvegarde. Champ System.control_mode (cloud|local). UI : carte « Pilotage local » (LocalManager.jsx) dans Paramètres. Packaging Docker : docker-compose.yml (+ .pi.yml host-network), Dockerfiles back/front, .env.example, guide RUN_LOCAL_WINDOWS.md. Endpoints validés via curl (dégradation gracieuse) + UI vérifiée par capture. NB : la validation RÉELLE des appareils (scan LAN + test) nécessite d'exécuter l'app sur le MÊME réseau que les appareils (PC maison ou Pi) — non testable depuis le serveur cloud Emergent.
- 2026-07-12 : Tri des appareils locaux (inclus/ignorés). Champ `local_devices.included` + endpoint PUT /admin/tuya/local/devices/{id}. classify_local_device → gainable/thermostat/autre. UI LocalManager : sections « Appareils du système » vs « Autres — ignorés » + bouton inclure/retirer (les appareils perso comme alarme/portillon sont écartés). Validé (curl + capture).
- 2026-07-12 : Association client/installateur par QR UNIQUEMENT. PairingPanel réduit au scan de QR (plus de liste de découverte → évite de sélectionner le mauvais appareil). Génération + impression des QR reste côté super admin (CatalogManager). Validé par capture.
- 2026-07-12 : Packaging auto-hébergé finalisé. docker-compose.yml (name: zoneclimate → volume stable/persistant), Dockerfiles (frontend sans yarn.lock, backend requirements.txt nettoyé 128→13 paquets), maj-zoneclimate.bat (updater 1-clic : git pull + docker compose up --build, localise git de GitHub Desktop), guide RUN_LOCAL_WINDOWS.md. Utilisateur (novice) a déployé avec succès sur son PC Windows via clone GitHub Desktop + updater 1-clic validé. Piège rencontré/résolu : .env écrit par Notepad (BOM/here-string) illisible par compose → fix via Set-Content -Encoding ascii avec `n.
- 2026-07-12 : Mise à jour intégrée (OTA) dans l'appli. Fichier VERSION (1.0.0). Endpoints GET /api/system/update-info (enabled, current_version, update_available via git dans REPO_DIR) + POST /api/system/update (lance un conteneur updater détaché : git pull + docker compose up -d --build via socket Docker). Bandeau UpdateBanner (admins) : « Application à jour / Mise à jour disponible » + boutons Vérifier / Installer. Piloté par env INAPP_UPDATE_ENABLED (défaut true → visible sur les Pi ; l'utilisateur met false sur son PC Windows). Montages socket+repo ajoutés dans docker-compose.pi.yml (REPO_DIR=/repo). SDK python `docker` ajouté. UI + endpoint testés sur cloud (apply réel = validé sur Pi/Linux uniquement).
- ROADMAP validée avec l'utilisateur (déploiement commercial) : (1) Provisioning central des clés (compte SmartLife ZoneClimate maître, appairage atelier, clés stockées super-admin, IP fixe → pas de whitelist chez client) ; (2) Affecter les appareils de la base super-admin à chaque installation client ; (3) Pi en point d'accès Wi-Fi « ZONING VALSON » (intégré) + Internet client via clé Wi-Fi USB (double radio) ; (4) Page onboarding Wi-Fi client ; (5) Image SD Raspberry prête à cloner (industrialisation) ; (6) Mise à jour OTA intégrée dans l'appli (bouton) sur le Pi/Linux.
