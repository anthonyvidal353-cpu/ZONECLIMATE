...
- 🚧 [2026-08] RF CONFIRMÉ comme voie retenue (raison BUSINESS : user veut REVENDRE le système → Tuya limite le nb d'appareils/compte + dépendance cloud). Alternatives écartées par contraintes user : Modbus RS485 (pas de thermostat E-TOP SANS FIL en Modbus), Zigbee (user trouve les thermostats Zigbee "moches" malgré argument "même design en version Zigbee"). User garde ses thermostats E-TOP 868 MHz + récepteurs Tuya (récepteurs conservés pour ouvrir les vannes, mais on abandonne le côté cloud Tuya → l'automate écoute/émet directement sur le lien RF 868 thermostat↔récepteur). CONFLIT "deux chefs" identifié pour l'émission (thermostat autonome ré-émet) → à gérer via décodage. IMPORTANT : le récepteur E-TOP est un pont Tuya Wi-Fi↔RF (signal aller/retour : app Tuya → récepteur → RF → thermostat met à jour son écran). Clé locale Tuya ne change JAMAIS sauf si on SUPPRIME l'appareil de SmartLife (=reset usine → oublie Wi-Fi + change clé). Setup Docker RF : backend/Dockerfile ajoute rtl-433+rtl-sdr DANS l'image ; docker-compose.pi.yml déjà privileged + /dev partagé (USB OK, rien à changer) ; install-pi.sh blackliste le pilote DVB-T (dvb_usb_rtl28xxu) pour libérer la clé. Endpoint GET /system/connectivity ajouté (test Internet). NOTE : outil Capture RF nécessite Save-to-GitHub (rebuild image) + update Pi + commande one-shot blacklist DVB sur l'hôte. PROCHAINE ÉTAPE : user a branché la clé → déployer → capturer (868.3, mode analyse) → coller le résultat → décoder E-TOP.
