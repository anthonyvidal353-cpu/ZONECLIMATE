...
- ✅ [2026-06] Pilotage du GAINABLE en Modbus RTU (RS485) via l'automate — CODE PRÊT (validable sur matériel) : module `modbus_gainable.py` (pymodbus 3.14, 9600/N/8/1, FC03/06/10). Registres : 0x0201 marche, 0x0202 mode (1=froid,4=chaud,5=auto), 0x0203 consigne ×0,1°C (160-310), 0x0204 ventilation (2/4/6/auto=1), 0x0318 temp ambiante (val-1000)/10. System.modbus_enabled/port/slave + endpoint POST /installations/{iid}/gainable/modbus/test (dégradation gracieuse hors matériel). Branché dans apply_local_control (gainable=Modbus, thermostats=Tuya). UI : GainableModbusDialog (activer/port/slave/test). Architecture confirmée : thermostats zones+vannes = Tuya Wi-Fi, gainable = Modbus, mode imposé par le thermostat maître. VALIDÉ testing_agent iteration_24 (100% backend) + encodage testé unitairement. NON testé sur matériel réel (pas de RS485 en cloud).
