...
- 2026-07-10 : ASSOCIATION PAR QR CODE (sans erreur). Catalogue QR (super_admin/moderator) : POST /admin/catalog/discover, GET /admin/catalog — génère un code unique (CZ-xxxx) + QR imprimable par appareil réel (collection 'catalog', tuya_id masqué). Association : POST /installations/{iid}/associate-qr (tout rôle avec droit d'écriture) — le QR identifie l'appareil exact, l'utilisateur choisit la zone ; gainable auto-rattaché à la zone maître ; refus double association (400) et QR inconnu (404). Frontend : onglet 'Catalogue QR' (impression étiquettes), scanner caméra html5-qrcode + repli manuel (QrAssociateDialog). Tests backend 15/15 (iteration_16) + frontend 100% (iteration_17). Flux d'appairage physique : ajout unique via l'app Smart Life (compte lié) → génération/collage QR par l'installateur → scan par le client pour associer.
